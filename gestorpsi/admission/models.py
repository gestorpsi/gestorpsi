...
class AdmissionManager(models.Manager):
    ...
    def inrange(self, organization, datetime_start=None, datetime_end=None):
        ...
        if not provided 'date start' set it as the creation date
        of the organization
        ...
        inrange = []
        for a in super(AdmissionManager, self).get_query_set().filter(
                client__person__organization=organization):
    ...
    # def signed(self, organization, query_pk_in=None):
        # q = super(AdmissionManager, self).get_query_set().filter(
        #    client__person__active=True, signed_bythe_client=True,
        #    client__person__organization=organization)
        # if query_pk_in:
    ...
    # def not_signed(self, organization, query_pk_in=None):
        # q = super(AdmissionManager, self).get_query_set(
        #    ).filter(client__person__active=True, signed_bythe_client=False,
        #     client__person__organization=organization)
        # if query_pk_in:
        # q = q.filter(pk__in=query_pk_in)
        # return q
...
class AdmissionInRangeManager(models.Manager):
    ...
    def all(self, organization, datetime_start=None, datetime_end=None):
        return AdmissionReferral.objects.inrange(organization, datetime_start,
                                                 datetime_end)
...
class Attach(models.Model):
    ...
    type = models.CharField(
        max_length=2, blank=True, null=True, choices=ATTACH_TYPE)
    client = models.ForeignKey(Client)
