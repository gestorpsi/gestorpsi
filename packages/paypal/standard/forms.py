...
from packages.paypal.standard.conf import *
from packages.paypal.standard.widgets import ValueHiddenInput, ReservedValueHiddenInput
from packages.paypal.standard.conf import (POSTBACK_ENDPOINT, SANDBOX_POSTBACK_ENDPOINT, 
    RECEIVER_EMAIL)
...
class PayPalPaymentsForm(forms.Form):
    ...
    def __init__(self, button_type="buy", *args, **kwargs):
        ...
        if 'initial' in kwargs:
            #raise Exception( kwargs['initial'] )
            for key, value in kwargs['initial'].iteritems():   # iter on both keys and values
                if key.startswith('amount'):
                    self.fields[key] = forms.IntegerField(widget=ValueHiddenInput())
                elif key.startswith('item_name'):
                    self.fields[key] = forms.CharField(widget=ValueHiddenInput())
                elif key.startswith('item_number'):
                    self.fields[key] = forms.CharField(widget=ValueHiddenInput())
...
class PayPalSharedSecretEncryptedPaymentsForm(PayPalEncryptedPaymentsForm):
    ...
    def __init__(self, *args, **kwargs):
        ...
        from packages.paypal.standard.helpers import make_secret
