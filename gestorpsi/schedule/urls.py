...
from gestorpsi.schedule.forms import ScheduleOccurrenceForm,\
    ScheduleSingleOccurrenceForm, OccurrenceConfirmationForm

from gestorpsi.schedule.views import occurrence_view, schedule_settings,\
    add_event, schedule_index, schedule_occurrence_listing_today, event_view,\
    daily_occurrences, today_occurrences, occurrence_confirmation_form,\
    occurrence_family_form, occurrence_employee_form, occurrence_group,\
    week_view, week_view_table
...
urlpatterns = patterns(
    ...
    url(
        r'^ place/(?P < place > ([a-f0-9]{8}-[a-f0-9]{4}-[a-f0-9]{4}-[a-f0-9]{4}-[a-f0-9] {12}))/$',  # noqa
        login_check(schedule_index),
        name='schedule-index'
    ),

    url(
        r'^ events/place/(?P<place>([a-f0-9]{8}-[a-f0-9]{4}-[a-f0-9]{4}-[a-f0-9]{4}-[a-f0-9]{12}))/$',  # noqa
    ...
    # return JSON
    url(
        r'^ occurrences/(?P<year>(\d)+)/(?P<month>(\d)+)/(?P<day>(\d)+)/place/(?P<place>([a-f0-9]{8}-[a-f0-9]{4}-[a-f0-9]{4}-[a-f0-9]{4}-[a-f0-9]\{12}))/$',  # noqa
    ...
    url(
        r'^ week/place/(\?P<place>([a-f0-9]{8}-[a-f0-9]{4}-[a-f0-9]{4}-[a-f0-9]{4}-[a-f0-9]\{12}))/$',  # noqa
        login_check(week_view),
        name='schedule-index'
    ),
    url(
        r'^ week/([a-f0-9]{8}-[a-f0-9]{4}-[a-f0-9]{4}-[a-f0-9]{4}-[a-f0-9]{12})/(\d{4})/(0?[1-9]|1[012])/([0-3]?\d)/$',  # noqa
    ...
    url(
        r'^ events/group/(?P<group_id>[a-f0-9]{8}-[a-f0-9]{4}-[a-f0-9]{4}-[a-f0-9]{4}-[a-f0-9]{12})/occurrence/(?P<occurrence_id>\d+)/$',  # noqa
